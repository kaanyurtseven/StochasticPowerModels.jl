...
for pen_level = pen_level_start:pen_level_step:pen_level_end
    ...
    time_SOTS[pen_level] = Dict()
    time_SOTS_bin[pen_level] = Dict()
    time_SOPF[pen_level] = Dict()

    stat_SOTS[pen_level] = Dict()
    stat_SOTS_bin[pen_level] = Dict()
    stat_SOPF[pen_level] = Dict()
        ...
        if solve_SOTS || solve_SOTS_bin || solve_SOPF
            ...
            if solve_SOTS || solve_SOTS_bin
                ...
                if solve_SOTS
                    ...
                    stat_SOTS[pen_level][iter] = string(result_SOTS[pen_level]["primal_status"])                    
                    time_SOTS[pen_level][iter] = result_SOTS[pen_level]["solve_time"]
                ...
                if solve_SOTS_bin
                    ...
                    stat_SOTS_bin[pen_level][iter] = string(result_SOTS_bin[pen_level]["primal_status"])
                    time_SOTS_bin[pen_level][iter] = result_SOTS_bin[pen_level]["solve_time"]
            ...
            if solve_SOPF
                ...
                stat_SOPF[pen_level][iter] = string(result_SOPF[pen_level]["primal_status"])
                time_SOPF[pen_level][iter] = result_SOPF[pen_level]["solve_time"]
...
if Report_time
    for pen_level = pen_level_start:pen_level_step:pen_level_end
        ...
        if solve_SOTS
        file_name1 = "Results\\Times\\Times - SOTS - $case ($pen_level).xlsx"
        fid    = XLSX.openxlsx(file_name1, mode="w")
        header = ["Iteration";"Status";"Time"]
        data   = [[collect(keys(time_SOTS[pen_level]))];[collect(values(stat_SOTS[pen_level]))];[collect(values(time_SOTS[pen_level]))]]
        XLSX.writetable(file_name1, data,header)
        end

        if solve_SOTS_bin
            file_name2 = "Results\\Times\\Times - SOTS (MINLP) - $case ($pen_level).xlsx"
            fid    = XLSX.openxlsx(file_name2, mode="w")
            header = ["Iteration";"Status";"Time"]
            data   = [[collect(keys(time_SOTS_bin[pen_level]))];[collect(values(stat_SOTS_bin[pen_level]))];[collect(values(time_SOTS_bin[pen_level]))]]
            XLSX.writetable(file_name2, data,header)
        end

        if solve_SOPF
            file_name3 = "Results\\Times\\Times - SOPF - $case ($pen_level).xlsx"
            fid    = XLSX.openxlsx(file_name3, mode="w")
            header = ["Iteration";"Status";"Time"]
            data   = [[collect(keys(time_SOPF[pen_level]))];[collect(values(stat_SOPF[pen_level]))];[collect(values(time_SOPF[pen_level]))]]
