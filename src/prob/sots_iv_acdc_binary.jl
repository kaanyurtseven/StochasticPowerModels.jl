...
""
function solve_sots_iv_acdc_binary(data::Dict, model_constructor, optimizer; deg::Int=1, p_size=0, solution_processors=[sol_data_model!], kwargs...)
    ...
    result = _PM.solve_model(data, model_constructor, optimizer, build_sots_iv_acdc_binary; multinetwork=true, ref_extensions = [_PMACDC.add_ref_dcgrid!, _SPM.add_ref_RES!], solution_processors=solution_processors, kwargs...)
...
""
function build_sots_iv_acdc_binary(pm::AbstractPowerModel)
    ...
    for (n, network) in _PM.nws(pm) 
        ...
        if _FP.is_first_id(pm,n,:PCE_coeff)
            variable_branch_indicator_binary(pm, nw=n)
            variable_dc_branch_indicator_binary(pm, nw=n)
        end
