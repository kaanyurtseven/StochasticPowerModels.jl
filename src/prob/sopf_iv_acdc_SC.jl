...
""
function solve_sopf_iv_acdc_SC(data::Dict, model_constructor, optimizer; deg::Int=1, p_size=0, solution_processors=[sol_data_model!], kwargs...)
    ...
    result = _PM.solve_model(data, model_constructor, optimizer, build_sopf_iv_acdc_SC; multinetwork=true, ref_extensions = [_PMACDC.add_ref_dcgrid!, _SPM.add_ref_RES!], solution_processors=solution_processors, kwargs...)
...
""
function build_sopf_iv_acdc_SC(pm::AbstractPowerModel)

    global curt_status = pm.data["curtailment"]
    global action_status = pm.data["action"]
    global gen_buses = pm.data["gen_buses"]
    ...
    for (n, network) in _PM.nws(pm)
        ...
        for i in _PM.ids(pm, :bus, nw=n)
            ...
            if (_FP.is_first_id(pm,n,:contingencies) == false) && (i in gen_buses)
                constraint_voltage_magnitude_link(pm, i, nw = n)
            end
        ...
        for g in _PM.ids(pm, :gen, nw=n)
            ...
            if action_status["Preventive Redispatch"] == true
                if _FP.is_first_id(pm,n,:PCE_coeff) #preventive re-dispatch
                    constraint_gp_gen_power_redispatch(pm, g, nw=n)
                end
            end

            if (_FP.is_first_id(pm,n,:contingencies) == false)
                constraint_gen_redispatch_definition(pm, g, nw=n)
            end
        end
        ...
        for i in _PM.ids(pm, :convdc, nw=n)
            ...
            if action_status["Preventive HVDC"] == true
                if _FP.is_first_id(pm,n,:PCE_coeff) #preventive HVDC
                    constraint_gp_converter_ac_power_redispatch(pm, i, nw=n)
                end
            end
        ...
        if _FP.is_first_id(pm,n,:PCE_coeff)
            ...
            for g in _PM.ids(pm, :gen, nw=n)
                ...
                if (_FP.is_first_id(pm,n,:contingencies) == false)
                    constraint_cc_gen_redispatch(pm, g, nw=n)
                end
            end
    ...
    objective_min_expected_generation_cost_SC(pm)
