...
""
function arsiv_solve_sots_iv_acdc(data::Dict, model_constructor, optimizer; deg::Int=1, p_size=0, solution_processors=[sol_data_model!], kwargs...)
    ...
    result = _PM.solve_model(data, model_constructor, optimizer, arsiv_build_sots_iv_acdc; multinetwork=true, ref_extensions = [_PMACDC.add_ref_dcgrid!, _SPM.add_ref_RES!], solution_processors=solution_processors, kwargs...)
...
""
function arsiv_build_sots_iv_acdc(pm::AbstractPowerModel)
    ...
    for (n, network) in _PM.nws(pm) 
        ...
        variable_branch_current_on_off_part1(pm, nw=n, bounded=bounded)

        variable_dcbranch_current_on_off(pm, nw=n, bounded=false)
        ###
    ...
    for (n, network) in _PM.nws(pm)
        ...
        for b in _PM.ids(pm, :branch, nw=n)
            ...
            constraint_gp_branch_series_current_on_off_definition(pm, b, nw=n)
            constraint_gp_branch_series_current_magnitude_squared_on_off(pm, b, nw=n)

            constraint_gp_branch_indicator(pm, b, nw=n)
        end
        ...
        for i in _PM.ids(pm, :branchdc, nw=n)
            ...
            constraint_gp_ohms_dc_branch_on_off(pm, i, nw=n) 
            constraint_ohms_dc_branch_on_off(pm, i, nw=n)

            constraint_gp_dc_branch_on_off_definition(pm, i, nw=n)
